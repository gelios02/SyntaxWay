...
def login():
    if request.method == "POST":
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            ...
            session.permanent = True  # Сессия будет сохраняться даже после перезагрузки
...
# Uncoment for filing train
# def fill_training_data():
#     with app.app_context():
#         for day in range(2, 30):
#             material = f"Test material for day {day}"
#             questions = f"Test questions for day {day}"
...
#                 material=material,
#                 questions=questions,
#                 answers="filled for test",
#                 correct_percentage=60.0,
#                 incorrect_percentage=40.0,
#                 recommendation="filled for test"
#             )
#             db.session.add(new_record)
#         db.session.commit()
#         print("TrainingData records inserted for days 3 to 29 for user 1, language Python.")
